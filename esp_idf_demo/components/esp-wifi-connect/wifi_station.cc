...
void WifiStation::Stop() {
    ...
    if (candidate_timer_handle_ != nullptr) {
        esp_timer_stop(candidate_timer_handle_);
        esp_timer_delete(candidate_timer_handle_);
        candidate_timer_handle_ = nullptr;
    }
...
void WifiStation::OnNoCandidates(std::function<void()> on_no_candidates) {
    on_no_candidates_ = std::move(on_no_candidates);
}
...
void WifiStation::Start() {
    ...
    esp_timer_create_args_t candidate_timer_args = {
        .callback = [](void* arg) {
            static_cast<WifiStation*>(arg)->HandleCandidateTimeout();
        },
        .arg = this,
        .dispatch_method = ESP_TIMER_TASK,
        .name = "WiFiCandidate",
    ...
    ESP_ERROR_CHECK(esp_timer_create(&candidate_timer_args, &candidate_timer_handle_));
}
...
void WifiStation::HandleScanResult() {
    ...
    std::vector<WifiPolicyScanResult> scan;
    scan.reserve(ap_records.size());
    for (const auto& ap_record : ap_records) {
        scan.push_back({
            std::string(reinterpret_cast<const char*>(ap_record.ssid),
                        strnlen(reinterpret_cast<const char*>(ap_record.ssid),
                                sizeof(ap_record.ssid))),
            ap_record.rssi,
        });
    }
    std::vector<WifiPolicyCredential> credentials;
    credentials.reserve(ssid_list.size());
    uint64_t priority = ssid_list.size();
    for (const auto& item : ssid_list) {
        credentials.push_back({item.ssid, item.password, priority--});
    }

    connect_queue_.clear();
    for (const auto& candidate : wifi_policy_rank_scan(scan, credentials)) {
        const wifi_ap_record_t* strongest = nullptr;
        for (const auto& ap_record : ap_records) {
            const std::string visible_ssid(
                reinterpret_cast<const char*>(ap_record.ssid),
                strnlen(reinterpret_cast<const char*>(ap_record.ssid),
                        sizeof(ap_record.ssid)));
            if (visible_ssid == candidate.ssid &&
                (strongest == nullptr || ap_record.rssi > strongest->rssi)) {
                strongest = &ap_record;
            }
        }
        ...
        WifiApRecord record = {
            ...
            .channel = strongest->primary,
            .authmode = strongest->authmode,
            .bssid = {0},
        };
        memcpy(record.bssid, strongest->bssid, sizeof(record.bssid));
        connect_queue_.push_back(record);
    }

    const auto first_pass = connect_queue_;
    connect_queue_.insert(connect_queue_.end(), first_pass.begin(), first_pass.end());

    if (connect_queue_.empty()) {
        ESP_LOGI(TAG, "No saved AP candidate visible");
...
void WifiStation::StartConnect() {
    ...
    {
        std::lock_guard<std::mutex> lock(candidate_mutex_);
        candidate_deadline_us_ = esp_timer_get_time() +
                                 static_cast<int64_t>(candidate_timeout_ms_) * 1000;
        candidate_active_ = true;
        if (candidate_timer_handle_ != nullptr) {
            esp_timer_stop(candidate_timer_handle_);
            esp_timer_start_once(candidate_timer_handle_,
                                 static_cast<uint64_t>(candidate_timeout_ms_) * 1000ULL);
        }
    }
}
...
void WifiStation::ScheduleNextScan() {
    ...
    if (rescan_epoch_us_ == 0) {
        rescan_epoch_us_ = now_us;
    }
    const int64_t deadline_ms = wifi_policy_next_rescan_deadline(
        rescan_epoch_us_ / 1000, rescan_index_++);
    const int64_t delay_us = std::max<int64_t>(1000, deadline_ms * 1000 - now_us);
    esp_timer_stop(timer_handle_);
    esp_timer_start_once(timer_handle_, static_cast<uint64_t>(delay_us));
    ESP_LOGI(TAG, "Next saved-network scan in %lld ms",
             static_cast<long long>(delay_us / 1000));
}

void WifiStation::HandleCandidateTimeout() {
    std::lock_guard<std::mutex> lock(candidate_mutex_);
    if (!candidate_active_) {
        return;
    }
    const int64_t remaining_us = candidate_deadline_us_ - esp_timer_get_time();
    if (remaining_us > 0) {
        esp_timer_start_once(candidate_timer_handle_, static_cast<uint64_t>(remaining_us));
        return;
    }
    candidate_active_ = false;
    ESP_LOGW(TAG, "Candidate timeout for %s; disconnecting", ssid_.c_str());
    esp_wifi_disconnect();
}
...
void WifiStation::SetCandidateTimeout(int timeout_ms) {
    candidate_timeout_ms_ = std::max(1, timeout_ms);
}
...
void WifiStation::WifiEventHandler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data) {
    ...
    if (event_id == WIFI_EVENT_STA_START) {
        this_->rescan_epoch_us_ = esp_timer_get_time();
        this_->rescan_index_ = 0;
        this_->StartScan();
        ...
        {
            std::lock_guard<std::mutex> lock(this_->candidate_mutex_);
            this_->candidate_active_ = false;
            if (this_->candidate_timer_handle_ != nullptr) {
                esp_timer_stop(this_->candidate_timer_handle_);
            }
        }
