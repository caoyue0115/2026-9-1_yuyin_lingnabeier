...
description: Use when preparing, enabling, validating, promoting, disabling, or rolling back OTA canary releases for the ESP-VoCat V1.0 ESP32-S3 N16R8 sample line in this repository.
---
...
- The applicable `docs/deploy/greenunion-sh-ota-*.md` runbook
- `scripts/v6_n16r8_release_gate.py --help`

Do not use an older repository or the deprecated V1.2 audio profile as the implementation source.
...
4. Run focused tests, firmware compile, `git diff --check`, the repository secret scan, and OTA drift/release guards.
...
10. Create a separate release for the next device only after the current canary passes.

## Automated Gate

Use placeholders in shared output:

```bash
python3 scripts/v6_n16r8_release_gate.py \
  --artifact tmp/<artifact>.bin \
  --expected-sha256 <sha256> \
  --expected-version v6-n16r8-<N>-ota-canary \
...
Report only public Git SHAs, artifact size, pass/fail booleans, counts, coarse reason enums, sanitized gate results, and the next authorized action. Never include protected runtime values or raw logs.
