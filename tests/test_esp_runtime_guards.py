...
class EspRuntimeGuardTests(unittest.TestCase):
    ...
    def test_followup_listener_starts_immediately_and_keeps_full_ten_seconds(self) -> None:
        ...
        self.assertIn(
            "armed_at_us = wait_start_us + (int64_t)arm_delay_ms * 1000",
            audio_in_c,
        )
    ...
    def test_v7_four_turn_controller_is_integrated(self) -> None:
        ...
        self.assertIn("CONVERSATION_DEFAULT_MAX_TURNS 4", header)
        self.assertIn("CONVERSATION_GAME_MAX_TURNS 10", header)
        self.assertIn("conversation_controller_set_max_turns", header)
        self.assertIn("controller->max_turns", source)
        ...
        self.assertIn("result.max_turns", main_c)
        self.assertIn("result.pet_mood", main_c)
